...
<?php if( $this->ordCalcObj->isThereSomeProductsOrdered()): ?>
    <?php foreach ($this->ordCalcObj->getProdotti() AS $idprodotto => $prodRow): 
        ...
        <div<?php if(!$pObj->isDisponibile()) { echo ' class="text-danger strike"';} ?>>
            ...
            <h3 class="big-margin-top"><strong><?php echo $pObj->getQtaOrdinata();?>/<?php echo $pObj->getQtaReale();?></strong> x <?php echo $pObj->descrizione;?></h3>
            <p>
            ...
            <?php foreach ($utenti AS $iduser => $user): ?>
                <li><?php echo $user["nome"] . " " . $user["cognome"]; ?> (<strong><?php echo $user["qta"]; ?>/<?php echo round($user["qta_reale"]); ?></strong>)</li>
            <?php endforeach; ?>
            </ul>        
